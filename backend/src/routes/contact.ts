...
const contactFormSchema = z.object({
  ...
  projectType: z.enum(['RESIDENTIAL', 'COMMERCIAL', 'INDUSTRIAL', 'RENOVATION', 'CONSULTATION']).optional(),
  budget: z.enum(['UNDER_10K', '10K_50K', '50K_100K', '100K_500K', 'OVER_500K']).optional(),
  timeline: z.enum(['ASAP', 'WITHIN_MONTH', 'WITHIN_QUARTER', 'WITHIN_YEAR', 'NO_RUSH']).optional(),
  location: z.string().optional(),
  source: z.enum(['WEBSITE', 'GOOGLE', 'SOCIAL_MEDIA', 'REFERRAL', 'ADVERTISEMENT', 'OTHER']).optional()
});

/**
 * Submit Contact Form
 */
router.post('/submit', optionalAuth, asyncHandler(async (req: Request, res: Response) => {
