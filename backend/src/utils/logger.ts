...
export const logEmailOperation = (operation: string, recipient?: string, template?: string, success?: boolean, error?: Error) => {
  const logData = {
    operation,
    recipient: recipient ? recipient.replace(/(.{2}).*@(.*)/, '$1***@$2') : undefined, // Mask email for privacy
    template,
    success,
    timestamp: new Date().toISOString()
  };
...
export const logRateLimit = (ip: string, endpoint: string, limit: number, current: number) => {
...
export const logSystemHealth = (component: string, status: 'healthy' | 'unhealthy' | 'degraded', metrics?: any) => {
