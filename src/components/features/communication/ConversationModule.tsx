...
export default function ConversationModule({
  ...
  const [messages, setMessages] = useState<Message[]>([
    ...
    },
    {
      ...
      sender: "Jane Smith",
      content:
        "Yes, I've prepared the latest reports. Should we schedule a call?",
  ...
  const {
    ...
    onMessage: (message) => {
      ...
      if (message.from !== user?.id && isNotificationEnabled) {
        showNotification("New Message", `${message.from}: ${message.content}`, {
          icon: "/favicon.ico",
    ...
    onCall: (call) => {
      ...
      if (call.type === "incoming") {
        showNotification("Incoming Call", `${call.from} is calling you`, {
          icon: "/favicon.ico",
          tag: `call-${call.id}`,
    ...
    onNotification: (notification) => {
      ...
      if (notification.type === "system") {
        const systemMessage: Message = {
          ...
          sender: "System",
          content:
            notification.message || notification.title || "System notification",
          timestamp: new Date(),
          type: "system",
        };
  ...
  return (
    <>
      <Card
        className={`w-full max-w-4xl mx-auto bg-white dark:bg-gray-800 shadow-lg ${className}`}
      >
                    ...
                    <div className="flex-1 relative">
                      ...
                      {attachmentPreview && (
                        <div className="absolute bottom-full left-0 right-0 mb-2 p-3 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 rounded-lg shadow-lg">
                          <div className="flex items-center justify-between">
                            <div className="flex items-center space-x-2">
                              {attachmentPreview.file.type.startsWith(
                                "image/",
                              ) ? (
                                <img
              ...
              <div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex items-center gap-3 p-2 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700/50">
                    ...
                    <Badge variant="secondary" className="text-xs">
                      <Users className="h-3 w-3 mr-1" />3
                    </Badge>
                  </div>
                </div>
              </div>
