...
import { Sidebar, SidebarProvider, SidebarHeader, SidebarContent, SidebarFooter, SidebarGroup, SidebarGroupLabel, SidebarGroupContent, SidebarMenu, SidebarMenuItem, SidebarMenuButton, SidebarSeparator, SidebarInset, SidebarTrigger } from "@/components/ui/sidebar";
...
const TeamCommunication: React.FC<TeamCommunicationProps> = ({ 
  ...
  const { sendMessage: wsendMessage, isConnected, joinRoom } = useWebSocket({
    onMessage: (message) => {
    ...
    onCall: (call) => {
      try {
        ...
        const fromName = (call as any)?.from?.name || (call as any)?.from || 'Unknown';
  ...
  const filteredUsers = useMemo(() => {
    ...
    if (selectedDept && selectedDept !== "All") {
      filtered = filtered.filter(u => (u.department || "Unassigned") === selectedDept);
    }
    if (userSearch.trim()) {
  ...
  return (
    <SidebarProvider>
      ...
      <SidebarInset>
        <Card className="w-full max-w-6xl mx-auto bg-white dark:bg-gray-900 shadow-2xl" ref={containerRef}>
      <CardHeader className="pb-3 border-b border-gray-200 dark:border-gray-800 bg-gradient-to-r from-orange-50 to-transparent dark:from-gray-800/40">
        <div className="flex items-center justify-between">
          <CardTitle className="text-lg font-semibold text-gray-900 dark:text-white flex items-center gap-2">
            ...
            {showModernUiBadge && (
              <Badge variant="secondary" className="text-[10px]">Modern UI</Badge>
            )}
          </CardTitle>
          <div className="flex items-center gap-2">
            <div className="flex items-center gap-1" aria-live="polite" aria-label={connectionStatus ? "Online" : "Offline"}>
              {connectionStatus ? (
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="ghost" size="sm" className="h-8 w-8 p-0" aria-label="More header actions">
              ...
              <DropdownMenuContent align="end" className="w-56">
                ...
                <DropdownMenuSeparator />
                <DropdownMenuItem onClick={() => handleStartCallType("voice")}>
                  <Phone className="mr-2 h-4 w-4" /> Start voice call
                </DropdownMenuItem>
                <DropdownMenuItem onClick={() => handleStartCallType("video")}>
                  <Video className="mr-2 h-4 w-4" /> Start video call
                </DropdownMenuItem>
                <DropdownMenuSeparator />
                <DropdownMenuItem onClick={() => setActiveTab("chat")}>Go to Chat</DropdownMenuItem>
                <DropdownMenuItem onClick={() => setActiveTab("calls")}>Go to Calls</DropdownMenuItem>
                <DropdownMenuSeparator />
                <DropdownMenuCheckboxItem
                ...
                <DropdownMenuSeparator />
                <DropdownMenuItem onClick={() => setShowControlsDrawer(true)}>
      ...
      <CardContent className="p-0">
        <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as "chat" | "calls")}>        
                ...
                <div className="h-full border rounded-lg bg-gray-50 dark:bg-gray-800 flex flex-col">
                  <div className="p-3 border-b border-gray-200 dark:border-gray-700">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-xs text-gray-500 dark:text-gray-400">{filteredUsers.length} members</span>
                    </div>
                  </div>
                ...
                <div className="h-full border rounded-lg bg-white dark:bg-gray-900 flex flex-col">
                  <div className="px-4 py-2 border-b border-gray-200 dark:border-gray-800 flex items-center justify-between">
                    <div className="text-sm text-gray-600 dark:text-gray-300">
                      Channel: <span className="font-medium">{department || "general"}</span>
                    </div>
                    <div className="flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400">
                      <Wifi className={`h-3.5 w-3.5 ${connectionStatus ? "text-green-500" : "text-red-500"}`} />
                  ...
                  <div className="flex-1 overflow-y-auto p-4 space-y-4">
                    {messages.map((message, index) => {
                      ...
                      const showDateSeparator = !previousMessage || !isSameDay(message.timestamp, previousMessage.timestamp);
                      return (
                        <div key={message.id}>
                          {showDateSeparator && (
                                ...
                                <>
                                  <div className="flex items-baseline gap-2 mb-1">
                                    <span className="text-sm font-medium text-gray-900 dark:text-white">
                                      {message.from === user?.id ? "You" : message.sender}
                                    </span>
                  ...
                  <div className="p-4 border-t border-gray-200 dark:border-gray-800">
                    ...
                    <div className="flex gap-2">
                        ...
                        <div className="absolute right-2 top-1/2 -translate-y-1/2 flex items-center gap-1">
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end" className="w-48">
                              <DropdownMenuLabel>Quick Actions</DropdownMenuLabel>
                              <DropdownMenuSeparator />
                              <DropdownMenuItem onClick={() => setShowEmojiDrawer(true)}>
                                <Smile className="mr-2 h-4 w-4" /> Add emoji
                              </DropdownMenuItem>
                              <DropdownMenuItem onClick={() => setShowAttachDrawer(true)}>
                      ...
                      <Button
                        ...
                        disabled={!newMessage.trim() || isChatLoading || !connectionStatus}
                    ...
                    <div className="mt-2 text-xs text-gray-500 dark:text-gray-400">Press Enter to send, Shift+Enter for a new line.</div>
                    {/* Emoji Drawer */}
                    <Drawer open={showEmojiDrawer} onOpenChange={setShowEmojiDrawer}>
                      <DrawerContent>
                        <DrawerHeader>
                        ...
                        <div className="px-4 pb-4">
                          <div className="grid grid-cols-10 gap-2">
                            {emojis.map((e) => (
                              <Button key={e} variant="ghost" className="h-9" onClick={() => handleEmojiSelect(e)} aria-label={`Insert ${e}`}>
                                {e}
                              </Button>
                            ))}
                          </div>
                    ...
                    <Drawer open={showAttachDrawer} onOpenChange={setShowAttachDrawer}>
                      <DrawerContent>
                        ...
                        <div className="px-4 pb-4 space-y-3">
                          <Button variant="default" onClick={handlePickFile} aria-label="Choose file">
                            <Paperclip className="mr-2 h-4 w-4" /> Choose file
                          </Button>
                          <p className="text-xs text-muted-foreground">Supported types: images, docs, videos.</p>
                        </div>
                        <DrawerFooter>
                          <DrawerClose asChild>
                            <Button variant="outline">Close</Button>
      ...
      <Drawer open={showControlsDrawer} onOpenChange={setShowControlsDrawer}>
        <DrawerContent>
          <DrawerHeader>
          ...
          <div className="px-4 pb-4 space-y-4">
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                <Button onClick={() => handleStartCallType("voice")} className="h-10" aria-label="Start voice call">
                  <Phone className="mr-2 h-4 w-4" /> Voice
                </Button>
                <Button onClick={() => handleStartCallType("video")} className="h-10" aria-label="Start video call">
            ...
            <div>
              ...
              <div className="flex gap-2">
                <Button variant="outline" onClick={() => setActiveTab("chat")} aria-label="Go to Chat">Chat</Button>
                <Button variant="outline" onClick={() => setActiveTab("calls")} aria-label="Go to Calls">Calls</Button>
                <Button variant="outline" onClick={() => setShowSettings(true)} aria-label="Open Settings">Settings</Button>
              </div>
            </div>
            <div>
