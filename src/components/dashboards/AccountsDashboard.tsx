...
interface AccountsRecurringPayment {
  id: string;
  description: string;
  amount: number;
...
export default function AccountsDashboard() {
  ...
  const [recurringPayments] = useState<AccountsRecurringPayment[]>([
    {
      id: "1",
  ...
  const handleFinanceChat = async () => {
    try {
      await customAction(
        'financeChat',
        async () => {
          // Simulate opening finance chat interface
          await new Promise(resolve => setTimeout(resolve, 1000));
          return { success: true };
        },
        'Finance chat opened successfully',
        'Failed to open finance chat'
      );
    } catch (error) {
      // Error is handled by customAction
    }
  };
