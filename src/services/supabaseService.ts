...
export async function register({ email, password, role = 'user', department }: RegisterPayload) {
...
export async function createTask(payload: { title: string; description?: string; assigned_to?: string }) {
...
export async function fetchMessages(conversationWith?: string) {
  ...
  if (conversationWith) q = q.or(`sender_id.eq.${conversationWith},receiver_id.eq.${conversationWith}`)
  const { data, error } = await q
  return { data, error }
}

export async function sendMessage(payload: { sender_id: string; receiver_id: string; message_text: string }) {
  const { data, error } = await supabase.from('messages').insert([payload])
  return { data, error }
}

export async function uploadFile(bucket: string, file: File | Blob, path: string) {
...
export async function updateUserDepartment(userId: string, department: string) {
  const { data, error } = await supabase.from('users').update({ department }).eq('id', userId)
  return { data, error }
}

export async function setUserBlocked(userId: string, blocked: boolean) {
  const { data, error } = await supabase.from('users').update({ blocked }).eq('id', userId)
  return { data, error }
}
...
export async function updateTestimonial(id: string, updateData: any) {
  const { data, error } = await supabase.from('testimonials').update(updateData).eq('id', id)
  return { data, error }
}
...
export async function updateProfile(profileData: any) {
  ...
  const { data: currentUserData, error: getUserErr } = await supabase.auth.getUser()
  ...
  if (profileData.email || profileData.password) {
    const { data, error } = await supabase.auth.updateUser({ email: profileData.email, password: profileData.password })
  ...
  if (Object.keys(updatePayload).length) {
    const { data, error } = await supabase.from('users').update(updatePayload).eq('auth_id', authId)
...
// File upload helpers
export async function uploadMultipleFiles(bucket: string, files: Array<File | Blob>, folder = '') {
  const uploaded: Array<{ path: string; error?: any }> = []
  for (let i = 0; i < files.length; i++) {
    const f = files[i]
    const filename = `file_${Date.now()}_${i}`
    const path = folder ? `${folder}/${filename}` : filename
    const { data, error } = await supabase.storage.from(bucket).upload(path, f as any)
...
export async function trackEvent(event: string, data?: any, userId?: string) {
  const { data: d, error } = await supabase.from('analytics').insert([{ event, data, user_id: userId }])
  return { data: d, error }
}
...
export async function getUserAnalytics(userId: string) {
  try {
    ...
    const { data, error } = await supabase.from('analytics').select('*').eq('user_id', userId).order('created_at', { ascending: false })
    return { data, error }
  }
}
...
export async function getLeadScoring() {
  try {
    ...
    const { data: fallback, error: e2 } = await supabase.from('contacts').select('status, id', { count: 'exact' })
