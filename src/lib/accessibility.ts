...
export class AccessibilityEngine {
  ...
  private async generateProjectAltText(imageUrl: string, context: string): Promise<string> {
    // Simulate AI analysis
    const projectTypes = ['residential building', 'commercial complex', 'infrastructure project', 'urban development'];
    const randomType = projectTypes[Math.floor(Math.random() * projectTypes.length)];
    
    const features = ['modern architecture', 'sustainable design', 'innovative construction', 'community-focused'];
  ...
  private async generateTeamAltText(imageUrl: string, context: string): Promise<string> {
    return 'Professional headshot of JD Marc Limited team member, contributing to African infrastructure development';
  }
  
  private async generateServiceAltText(imageUrl: string, context: string): Promise<string> {
    const services = ['construction equipment', 'architectural planning', 'infrastructure development', 'project management'];
  ...
  private initializeVoiceCommands(): void {
    ...
    this.voiceCommands.set('go to portfolio', {
      phrases: ['go to portfolio', 'show portfolio', 'view projects', 'see our work'],
    ...
    this.voiceCommands.set('go to contact', {
      phrases: ['go to contact', 'contact us', 'get in touch', 'book consultation'],
    ...
    this.voiceCommands.set('go to services', {
      phrases: ['go to services', 'show services', 'what do you do', 'our services'],
    ...
    this.voiceCommands.set('increase text size', {
      phrases: ['increase text size', 'make text larger', 'bigger text', 'zoom in'],
    ...
    this.voiceCommands.set('decrease text size', {
      phrases: ['decrease text size', 'make text smaller', 'smaller text', 'zoom out'],
    ...
    this.voiceCommands.set('high contrast', {
      phrases: ['high contrast', 'increase contrast', 'better contrast', 'dark mode'],
  ...
  private readPageContent(): void {
    ...
    paragraphs.forEach((paragraph, index) => {
      if (index < 2 && paragraph.textContent && paragraph.textContent.length > 20) {
  ...
  private adjustTextSize(direction: 'increase' | 'decrease'): void {
    ...
    this.speak(`Text size ${direction === 'increase' ? 'increased' : 'decreased'}`);
  }
  ...
  private getImageContext(img: HTMLImageElement): string {
    ...
    const parent = img.closest('section, article, div[class*="portfolio"], div[class*="team"], div[class*="service"]');
    if (parent) {
      const className = parent.className.toLowerCase();
      if (className.includes('portfolio') || className.includes('project')) return 'portfolio';
      if (className.includes('team') || className.includes('about')) return 'team';
...
export const useAccessibility = () => {
  return {
    startVoiceCommands: accessibilityEngine.startVoiceCommands.bind(accessibilityEngine),
    stopVoiceCommands: accessibilityEngine.stopVoiceCommands.bind(accessibilityEngine),
    listCommands: accessibilityEngine.listAvailableCommands.bind(accessibilityEngine),
    speak: accessibilityEngine.speak.bind(accessibilityEngine),
    generateAltText: accessibilityEngine.generateAltText.bind(accessibilityEngine),
