...
export const CallProvider: React.FC<CallProviderProps> = ({ children }) => {
  ...
  const declineIncomingCall = () => {
    try {
      ...
      if (callState.incomingCall) {
        ...
        console.log(
          `Declining call from department: ${department}, callId: ${callId}`,
        );
      } else {
  ...
  const simulateIncomingCall = (from: CallParticipant, department: string) => {
    try {
      console.log(
        `Simulating incoming call from: ${from.name}, department: ${department}`,
      );
      ...
      if (user?.department === department && !callState.isInCall) {
        ...
        console.log(
          `Incoming call not applicable - department mismatch or already in call`,
        );
      }
    } catch (error) {
  ...
  const simulateCallNotifications = (
    ...
    try {
      console.log(
        `Setting up call notification for department: ${department}, from: ${caller.name}`,
      );
      ...
      setTimeout(() => {
        try {
          console.log(
            `Checking if notification should be sent to current user`,
          );
          if (user?.department === department && user.id !== caller.id) {
            console.log(
              `Sending notification to user ${user.id} in department ${department}`,
            );
